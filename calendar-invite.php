...
if ( !function_exists( 'wp_mail' ) ) :
    ...
    function wp_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
        ...
        $icalphpmailer->clearAllRecipients();
        $icalphpmailer->clearAttachments();
        $icalphpmailer->clearCustomHeaders();
        $icalphpmailer->clearReplyTos();
        ...
        try {
            $icalphpmailer->setFrom( $from_email, $from_name, false );
        ...
        $icalphpmailer->Subject = $subject;
        $icalphpmailer->Body    = $message;
            ...
            foreach ( (array) $addresses as $address ) {
                try {
                    ...
                    switch ( $address_header ) {
                        case 'to':
                            $icalphpmailer->addAddress( $address, $recipient_name );
                            break;
                        case 'cc':
                            $icalphpmailer->addCc( $address, $recipient_name );
                            break;
                        case 'bcc':
                            $icalphpmailer->addBcc( $address, $recipient_name );
                            break;
                        case 'reply_to':
                            $icalphpmailer->addReplyTo( $address, $recipient_name );
                            break;
                    }
                } catch ( phpmailerException $e ) {
                    continue;
        ...
        $icalphpmailer->ContentType = $content_type;

        // Set whether it's plaintext, depending on $content_type
        if ( 'text/html' == $content_type )
            $icalphpmailer->isHTML( true );
         ...
         */
        $icalphpmailer->CharSet = apply_filters( 'wp_mail_charset', $charset );

        // Set custom headers
        if ( !empty( $headers ) ) {
            foreach ( (array) $headers as $name => $content ) {
                $icalphpmailer->addCustomHeader( sprintf( '%1$s: %2$s', $name, $content ) );
            }

            if ( false !== stripos( $content_type, 'multipart' ) && ! empty($boundary) )
                $icalphpmailer->addCustomHeader( sprintf( "Content-Type: %s;\n\t boundary=\"%s\"", $content_type, $boundary ) );
        }

        if ( !empty( $attachments ) ) {
            foreach ( $attachments as $attachment ) {
                try {
                    $icalphpmailer->addAttachment($attachment);
                } catch ( phpmailerException $e ) {
                    continue;
                }
            }
         ...
         * @since 2.2.0
         *
         * @param \PHPMailerCalendarInvite $icalphpmailer The PHPMailer instance (passed by reference).
         */
        do_action_ref_array( 'phpmailer_init', array( &$icalphpmailer ) );

        // Send!
        try {
            return $icalphpmailer->send();
        } catch ( phpmailerException $e ) {
